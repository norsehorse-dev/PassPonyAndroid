...
| NDK path seen by cargo-ndk | `/tmp/passpony-ndk` (symlink to the real install) | `scripts/build-core.sh` -- OpenSSL embeds the full CC path into libcrypto's build-info, so every host must present the NDK at the same path (see "What else makes the build deterministic" below) |
...
| `SOURCE_DATE_EPOCH` for PassPonyCore | `1786387087`, forced (the exact value matters -- see below) | PassPonyCore's own `.cargo/config.toml` (`[env]` table, `force = true`) |
...
`692d568...` is the current PassPonyCore HEAD rather than the `v1.0.0` tag,
...
- **OpenSSL's embedded build timestamp.** Once the `target-dir` pin above
  ...
  wall-clock time. OpenSSL honors `SOURCE_DATE_EPOCH` for this
  (`openssl/openssl#4639`), but the value is FORMATTED into the date
  string (`gmtime`), not replaced with a fixed placeholder -- so
  byte-identity requires every builder to use the exact same epoch, not
  merely have the variable set. An earlier revision of this section
  claimed "any value works" and PassPonyCore used `force = false`
  accordingly; F-Droid's buildserver, which exports `SOURCE_DATE_EPOCH`
  from the commit timestamp of the repo it's building, then overrode
  the pin and its rebuild embedded a different date than the published
  release -- caught by F-Droid's own reproducible-build comparison on
  the fdroiddata MR pipeline (2026-08-11, see the result log below).
  PassPonyCore's `.cargo/config.toml` now pins it via cargo's `[env]`
  table (which does reach build-script subprocesses, including
  `openssl-src`'s `perl Configure`/`make` invocations) with
  `force = true`, so the pinned epoch wins everywhere unconditionally.
...
- **The NDK's install path.** Same host-class isn't enough either:
  OpenSSL records the full CC invocation -- the NDK's absolute path
  included -- into libcrypto's build-info (`crypto/buildinf.h`'s
  `compiler:` line). The Docker container and GitHub CI both have the
  SDK at `/usr/local/lib/android/sdk` (deliberately matched), but
  F-Droid's buildserver has it at `/opt/android-sdk`, and that one
  embedded path difference failed F-Droid's reproducible-build
  comparison (2026-08-11, result log below). Fixed in
  `scripts/build-core.sh`: it symlinks the real NDK to
  `/tmp/passpony-ndk` and exports that as `ANDROID_NDK_HOME` before
  cargo-ndk runs -- cargo-ndk 4.1.2 uses the value verbatim without
  canonicalizing (verified against its source), so every host embeds
  the same neutral path no matter where the NDK actually lives.
...
| 2026-08-11 | F-Droid reproducible-build comparison (fdroiddata MR pipeline, `Binaries:` + `AllowedAPKSigningKeys:`), buildserver rebuild of `v1.0.1` vs the published foss APK | DIFFERS -- `libpass_ffi.so` on both ABIs, `classes.dex` clean; the recipe itself built successfully end to end, this was purely the byte comparison. Root-caused via `strings` diff of the buildserver's kept artifact against the published APK: exactly two embedded-string differences. (1) `built on:` -- mkbuildinf.pl FORMATS `SOURCE_DATE_EPOCH` into the date rather than emitting a fixed placeholder, so the buildserver's commit-timestamp value produced a different date than the pinned value (the pin was `force = false` and lost). (2) `compiler:` -- the NDK's absolute install path, `/usr/local/lib/android/sdk` (Docker/CI) vs `/opt/android-sdk` (buildserver). Both fixed for v1.0.2: PassPonyCore `692d568` forces the epoch pin, and `scripts/build-core.sh` normalizes the NDK path through the `/tmp/passpony-ndk` symlink. |
| | CI legs (JDK 17 / 21), `.github/workflows/reproducible.yml` on tag `v1.0.1` | pending -- triggered automatically by the tag push; superseded in practice by v1.0.2, log if checked |
